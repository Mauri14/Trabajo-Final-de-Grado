...
En \citep{gutierrez} (CITAR ANDRES GUTIERREZ) se introduce la siguiente definición de dominios.
...
\textit{Plotly} \citep{plotly} hace los gráficos interactivos, cuenta con varias opciones para poder visualizarlos de manera más precisa y permite descargarlos como imagen. (CITAR PAQUETE)
