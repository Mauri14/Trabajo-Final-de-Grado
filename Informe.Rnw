...
Este estimador es insesgado para el total $t=\sum_{U}^{}y_i$ \citep{sarndal2003model}. Por otra parte, el estimador de ratio o cociente $R$ entre dos variables queda definido:
